...
export function SessionVideoReview({ events }: { events: AIFeedbackEvent[] }) {
      ...
      <div className="space-y-4">
        ...
        {selectedEvent ? (
          <div className="bg-blue-50 border border-blue-200 p-6 rounded-2xl shadow-sm">
            <h3 className="text-xl font-semibold text-blue-900 mb-4">Why did the AI make this suggestion?</h3>
            <div className="grid grid-cols-2 gap-4 text-sm text-gray-800">
              <div>
                <p className="font-semibold text-gray-500">Suggestion:</p>
                <p className="font-bold">{selectedEvent.suggestion}</p>
              </div>
              <div>
                <p className="font-semibold text-gray-500">Video Timestamp:</p>
                <p>{formatTime(selectedEvent.videoTimeMs)} (Repetition {selectedEvent.repetitionNumber})</p>
              </div>
              ...
              <div className="col-span-2 text-xs text-gray-400 mt-2 flex justify-between">
                ...
                <span className="text-orange-500 font-bold">Status: Demo recommendation — therapist review required.</span>
              </div>
            </div>
          </div>
        ) : (
